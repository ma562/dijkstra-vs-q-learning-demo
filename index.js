...
let success = 0;
let testing_success = 0;
let TESTING = false;
let TIMEOUT = false;
let timeout_iteration = 0;
let TIMEOUT_CUTOFF = 0;
let DOWNLOAD = false;
let escape_path_length = []
let total_escape_paths = 0;
let caught_path_length = []
let total_caught_paths = 0;
...
TIMEOUT_CUTOFF = max_distance * 3;    //IF WE HAVE RUN 3 TIMES THE MAX DISTANCE THEN TIMEOUT
  ...
  for (let col = 1; col < map[row].length - 1; col++) {
    if (map[row][col] === ' ') {
      ...
      for (let catDirection of possibleCatDirections) {
        //note min distance is now 2 because the row and col array will always have a -1 at the end. eg. [x, -1]
        for (let distance = 2; distance <= pathLengthMatrix[row - 1][col - 1]; distance++) {
...
function createAndDownloadCSV(arrayData, fileName) {
    // Convert array to CSV string
    const csvContent = arrayData.map(e => e.toString()).join("\n");
  ...
  if(animate_iteration % UPDATE_FREQUENCY === 0) {
    ...
    if(TESTING && (restart || restart2)) {
      ...
      if(TESTING_EPISODES % 200 === 0) {
        console.log("TESTING PHASE DONE");
        console.log("testing episode:")
        console.log(TESTING_EPISODES);
        console.log("todays success");
        console.log(testing_success/200);
        if(testing_success === 0) {
          escape_path_length.push(-1);
        }
        else {
          escape_path_length.push(total_escape_paths / testing_success);    //total number of escape paths/ number of times escaped
        }
        if(testing_success === 200) {
          caught_path_length.push(-1);
        }
        else {
          caught_path_length.push(total_caught_paths / (200 - testing_success));  //total number of caught paths / number of times caught
        }
        total_path_length.push((total_escape_paths + total_caught_paths) / 200);
        success_rate.push(testing_success/200);
        testing_success = 0;
        TESTING = false;
        //reset our total count of paths
        total_escape_paths = 0;
        total_caught_paths = 0;

        // if(DOWNLOAD) {

        //   // Now you can call the function with different data and filenames
        //   createAndDownloadCSV(escape_path_length, 'average_escape_length.csv');
        //   createAndDownloadCSV(caught_path_length, 'average_caught_length.csv');
        //   createAndDownloadCSV(success_rate, 'success_rate.csv');
        //   createAndDownloadCSV(total_path_length, 'average_total_length.csv');
        //   saveQTable(Qtable);
        //   DOWNLOAD = false;
        // }
      }
      ...
      if(EPISODES % 50 === 0) {
        // console.log("TRAINING PHASE DONE");
        console.log(epsilon);
